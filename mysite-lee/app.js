...
const { ForbiddenError, ServiceError, UnknownError } = require('./utils/errors'); //引入禁止访问错误
...
// 配置验证token接口 验证通过之后才会进入后面的路由进行访问
app.use(expressJWT({
    secret: md5(process.env.JWT_SECRET), //服务器设置的密钥
    algorithms: ['HS256'], //新版本的expressJWT必须指定算法
}).unless({
    // 需要排除的token验证路由 就是不需要token就可以访问的
    path: [
...
// error handler 错误处理方法
app.use(function(err, req, res, next) {
    console.log("err.name>>>>", err.name);
    console.log("err.message>>>>>", err.message);
    if (err.name === 'UnauthorizedError') {
        res.send(new ForbiddenError("未登录/登录过期").toResponseJSON())
